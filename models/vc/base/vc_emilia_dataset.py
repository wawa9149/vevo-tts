...
from text.text_token_collation import TextTokenCollator
...
class VCEmiliaDataset(EmiliaDataset):
    def __init__(self, cfg, is_valid=False):
        ...
        # Prepare Korean phone-id collator from symbols_dict (no BOS/EOS)
        self.pad_phone_id = 1023  # must not collide with real tokens
        self.ko_token_collator = None
        try:
            symbols_dict_file = getattr(self.cfg.preprocess, "symbols_dict_file", None)
            if symbols_dict_file is None:
                processed_dir = getattr(self.cfg.preprocess, "processed_dir", None)
                symbols_name = getattr(self.cfg.preprocess, "symbols_dict", None)
                if processed_dir is not None and symbols_name is not None:
                    candidate = os.path.join(processed_dir, symbols_name)
                    if os.path.exists(candidate):
                        symbols_dict_file = candidate

            if symbols_dict_file is not None and os.path.exists(symbols_dict_file):
                base_tokens = SymbolTable.from_file(symbols_dict_file).symbols
                # Build a collator without BOS/EOS so sequences match VC expectations
                self.ko_token_collator = TextTokenCollator(
                    base_tokens, add_bos=False, add_eos=False
                )
                # Safety: ensure vocab size fits embedding/pad convention if used downstream
                if len(self.ko_token_collator.idx2token) >= self.pad_phone_id:
                    logger.warning(
                        f"Phone vocab size {len(self.ko_token_collator.idx2token)} exceeds or equals pad id {self.pad_phone_id}.\n"
                        "Please shrink vocab or increase embedding size and update padding assumptions (1024/1023)."
                    )
            else:
                if self.load_phone:
                    logger.warning(
                        "symbols_dict file not found; Korean phone-id mapping will not be available."
                    )
        except Exception as e:
            logger.warning(f"Failed to initialize Korean phone-id collator: {e}")
    ...
    def g2p(self, text, language):
        ...
        if language == "ko":
            # Route to MaskGCT-compatible KO G2P → IPA → ID mapper
            from models.tts.maskgct.g2p.utils.g2p import phonemizer_g2p
            ipa_str, token_ids = phonemizer_g2p(text, language)
            return [ipa_str, token_ids]
        else:
